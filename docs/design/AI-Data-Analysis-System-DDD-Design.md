...
| datasourceId | 非空，数据源必须存在且在线 | AppService + Gateway | 返回错误提示 |
...
| ----- | -------- | ------------------ | ------- | -------------------- |
| `NLQuerySubmitted` | 用户提交自然语言查询 | SemanticLayer | `{queryId, queryText, datasourceId, userId, timestamp}` | SQLGenerationPolicy |
...
| **数据源上下文 (Data Source Context)** | Generic Supporting（通用支撑） | DataSource, TableMetadata, ColumnMetadata | 多数据源配置、数据字典、连接管理 |
...
| --------------------- | ------------ | ---------- |
| 语义层上下文 → 语义模型上下文 | Customer-Supplier | 语义层查询时需要从语义模型获取指标定义 |
| 语义模型上下文 → 质量治理上下文 | Customer-Supplier | 指标变更后需要触发质量检查 |
| 质量治理上下文 → 数据流水线上下文 | Customer-Supplier | 质量通过后自动触发下游流程执行 |
| 语义层上下文 → 数据源上下文 | Customer-Supplier | SQL执行时需要从数据源读取表结构元数据 |
...
| 目录上下文 → 数据源上下文 | Conformist | 目录使用数据源的元数据信息 |
...
| `QueryRecord` | `entity` | `id` | userId, queryText, sqlText, confidence, status | submit, confirm, correct | SUBMITTED, GENERATED, CONFIRMED, CORRECTED, EXECUTED |
...
| SemanticLayer | DataSource | Reference by ID | N:1 | Yes |
...
| Permission | DataFlow/DataSource/MetricDefinition | Reference by ID | N:1 | Yes |
...
flowchart LR
    subgraph Publish
        SL[SemanticLayer] -->|publishes| E1[NLQuerySubmittedEvent]
        SL -->|publishes| E2[SQLGeneratedEvent]
        SL -->|publishes| E3[SQLCorrectedEvent]
        DF[DataFlow] -->|publishes| E4[FlowLayerExecutedEvent]
        QG[QualityGate] -->|publishes| E5[QualityGatePassedEvent]
        QG -->|publishes| E6[QualityGateFailedEvent]
        SM[SemanticModel] -->|publishes| E7[MetricDefinedEvent]
    end

    subgraph Subscribe
        E1 -->|handled by| SQLGenPolicy[SQLGenerationPolicy]
        E2 -->|handled by| ConfPolicy[ConfidenceCheckPolicy]
        E3 -->|handled by| KnowExtractPolicy[KnowledgeExtractionPolicy]
        E4 -->|handled by| QualityCheckPolicy[QualityCheckPolicy]
        E5 -->|handled by| DownstreamPolicy[DownstreamTriggerPolicy]
        E6 -->|handled by| AlertPolicy[AlertPolicy]
        E7 -->|handled by| RefreshPolicy[DownstreamRefreshPolicy]
    end
...
这三个领域被划分为**核心域**，体现了设计的核心价值主张。数据流水线、数据源、报表、可观测性等作为支撑域，权限作为横切通用域。划分合理。

**2. 聚合设计是否满足DDD原则？**

- 每个聚合有明确的Aggregate Root（DataFlow, SemanticLayer, QualityGate, SemanticModel, DataSource, Permission）
...
核心业务流程（NL查询→SQL生成→流程执行→质量门禁）均有对应的事件驱动设计：
