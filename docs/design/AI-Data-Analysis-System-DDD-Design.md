...
| `GenerateBusinessFormCommand` | Application | 业务人员/数据分析师 | BusinessForm | BusinessFormGenerated | AI生成业务表单 |
| `SaveFormTemplateCommand` | Application | 业务人员/数据分析师 | BusinessForm | FormTemplateSaved | 保存表单为模板 |
| `DefineBusinessDomainCommand` | Application | 管理员 | BusinessDomain | BusinessDomainDefined | 定义业务领域 |
...
| `BusinessFormGenerated` | AI生成业务表单完成 | BusinessForm | `{formId, formName, fields, businessDomain, generatedBy, timestamp}` | — |
| `FormTemplateSaved` | 表单模板保存完成 | BusinessForm | `{templateId, formName, businessDomain, savedBy, timestamp}` | — |
| `BusinessDomainDefined` | 业务领域定义完成 | BusinessDomain | `{domainId, domainName, parentDomainId, timestamp}` | — |
...
| --------------- | ----------------------- | ------------ | ----------- |
| **业务表单上下文 (Business Form Context)** | Core Domain（核心域） | BusinessForm, FormTemplate, BusinessDomain | AI生成业务表单、表单模板库、业务领域分类（采购/销售/项目/财务等） |
...
| ------ | ------- | -------------- | -------------- | --------- | ---------------- |
| `BusinessDomain` | `entity` | `id` | domainName, parentDomainId, description | define, addChild, getHierarchy | ACTIVE, INACTIVE |
| `BusinessForm` | `entity` | `id` | formName, businessDomainId, fields, status | generate, edit, saveAsTemplate, publish | DRAFT, PUBLISHED, ARCHIVED |
| `FormTemplate` | `entity` | `id` | templateName, businessDomainId, fields, usageCount | create, apply, updateUsageCount | ACTIVE, ARCHIVED |
| `FormField` | `entity` | `id` | formId, fieldName, fieldType, isRequired, defaultValue, validationRule | create, update, remove, reorder | ACTIVE, INACTIVE |
| `BusinessTable` | `entity` | `id` | name, businessDomainId, datasourceId, status, currentVersion | define, updateVersion, addColumn, syncData | DRAFT, ACTIVE, SYNCING, DEPRECATED |
...
| ---- | --- | ------------ | ------------ | --------------- |
| BusinessDomain | BusinessDomain | Self-Reference | 1:N | No（层级结构） |
| BusinessDomain | BusinessTable | Association | 1:N | Yes |
| BusinessDomain | BusinessForm | Association | 1:N | Yes |
| BusinessDomain | FormTemplate | Association | 1:N | Yes |
| BusinessForm | FormField | Composition | 1:N | No（同一聚合） |
| BusinessForm | FormTemplate | Reference by ID | N:1 | Yes（基于模板生成） |
| FormTemplate | FormField | Composition | 1:N | No（同一聚合） |
| BusinessTable | BusinessTableColumn | Composition | 1:N | No（同一聚合） |
| BusinessTable | BusinessTableVersion | Composition | 1:N | No（同一聚合） |
| BusinessTable | BusinessDomain | Reference by ID | N:1 | Yes（业务领域分类） |
...
| Permission | BusinessTable/DataFlow/MetricDefinition/BusinessForm | Reference by ID | N:1 | Yes |
...
erDiagram
    ...
    BusinessDomain ||--o{ BusinessDomain : "has children"
    BusinessDomain ||--o{ BusinessTable : "classifies"
    BusinessDomain ||--o{ BusinessForm : "classifies"
    BusinessDomain ||--o{ FormTemplate : "classifies"

    BusinessDomain {
        long id PK
        string domainName
        long parentDomainId FK "self-reference"
        string description
        string status
        datetime createdAt
    }

    %% ========== Aggregate: BusinessForm ==========
    BusinessForm ||--o{ FormField : "contains"
    BusinessForm }o..|| FormTemplate : "based on"
    BusinessForm }o..|| BusinessDomain : "belongs to"

    BusinessForm {
        long id PK
        string formName
        long businessDomainId FK
        long templateId FK "optional"
        string status
        datetime createdAt
        datetime updatedAt
    }
    ...
    BusinessTable }o..|| BusinessDomain : "belongs to"
...
*文档生成日期：2026-03-30*
*来源：AI数据分析系统PRD.md*
*版本：v1.2 — 新增业务表单上下文（BusinessForm、FormTemplate、BusinessDomain）；AI业务表单生成；业务表支持按业务领域分类（采购、销售、项目、财务等）*
